...
echo "       git -C $APP_DIR remote set-url origin $REPO_SSH"
echo "  2. Copy .env dari VPS lama (jika migrasi) atau buat .env baru:"
echo "       cd $APP_DIR/backend && php artisan config:clear"
echo "  3. Pindah DNS di Cloudflare: A fif-broadcast.net + www -> <NEW_IP>"
echo "  4. Setelah DNS propagate, buat SSL:"
echo "       certbot --nginx -d $DOMAIN -d www.$DOMAIN"
echo "       bash $APP_DIR/deploy/deploy-vps.sh $DOMAIN"
echo "  5. Autodeploy: tambahkan GitHub Actions secrets (VPS_HOST, VPS_USER, VPS_SSH_KEY)"
echo "======================================================"
