...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.replace(freshUser.role === 'superadmin' || freshUser.role === 'UH' ? '/admin/dashboard' : '/marketing/dashboard');
    } catch (err: unknown) {
      ...
      <div className="relative flex flex-col items-center gap-10">
        ...
        >
          ...
          <p className="pt-2 text-center text-sm text-slate-600">
            Belum punya akun?{' '}
            <Link href="/register" className="font-medium text-fif-400 transition-all hover:text-fif-300 hover:underline underline-offset-2">
              Daftar
            </Link>
          </p>
        </form>
