...
import { Upload, UserCheck, Search, Download, Link, FileSpreadsheet, Type, AlertCircle, CheckCircle2, Eye, Trash2, Filter, ChevronDown, User, Store, Users, ArrowRight, GitBranch } from 'lucide-react';
...
export default function CustomerManagementPage() {
  ...
  const [distributeKiosId, setDistributeKiosId] = useState('');
  const [distributeResult, setDistributeResult] = useState<{ message: string; distributed: number; per_kios: Record<string, { total: number; uh_count: number; per_uh: Record<string, { nmc: number; refi: number; other: number }> }> } | null>(null);
          ...
          <div className="flex items-center gap-2">
            <select
              ...
              className={`max-w-[130px] rounded-lg border px-2 py-1 text-xs font-semibold uppercase tracking-wider outline-none transition-all focus:ring-2 focus:ring-fif-500/30 ${
                coriVal === 'BAD' ? 'border-red-300 bg-red-50 text-red-700' :
  ...
  return (
    <div className="space-y-5 animate-fade-in">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {(user?.role === 'superadmin' || user?.role === 'AO') && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-purple-200 dark:border-purple-900/40 bg-white dark:bg-slate-800 px-3 py-2 text-xs font-medium text-purple-600 dark:text-purple-400 transition-all hover:border-purple-300 hover:bg-purple-50 hover:text-purple-700 dark:hover:border-purple-600 dark:hover:bg-purple-900/20"
            >
      ...
      <Modal open={showDistribute} onClose={() => { if (!distributing) { setShowDistribute(false); setDistributeResult(null); } }} title="Distribute ke UH">
        <div className="space-y-4">
          {!distributing && !distributeResult && (
            <>
              <div>
                <label className="mb-1.5 block text-xs font-medium text-slate-600 dark:text-slate-400">Pilih Kios</label>
                <select
                  value={distributeKiosId}
                  onChange={(e) => setDistributeKiosId(e.target.value)}
                  className="w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 py-2.5 text-sm outline-none transition-all focus:border-fif-500 focus:ring-2 focus:ring-fif-500/20"
                >
                  <option value="">Semua Kios ({Object.values(kiosDataCounts).reduce((a, b) => a + b, 0).toLocaleString('id-ID')} data)</option>
                  {kiosList.map((k) => {
                    ...
                    return (
                      <option key={k.kios_id} value={k.kios_id}>
                        {k.kios_name} ({k.kios_id}){count !== null ? ` — ${count.toLocaleString('id-ID')} data` : ''}
                      </option>
                    );
                  })}
                </select>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="secondary" onClick={() => { setShowDistribute(false); }}>
                  Batal
                </Button>
                <Button
                  variant="primary"
                  onClick={async () => {
                    ...
                    try {
                      const res = await customerService.distributeToUh(distributeKiosId || undefined);
                      setDistributeResult(res);
                      fetchData();
                    } catch {
                      toast('error', 'Gagal mendistribusikan data');
          ...
          {distributing && !distributeResult && (
            <div className="flex flex-col items-center gap-3 py-8">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Mendistribusikan data...</p>
            </div>
          )}
          {distributeResult && (
            <>
              <div className={`flex items-start gap-3 rounded-xl border p-4 ${
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{distributeResult.message}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    {distributeResult.distributed.toLocaleString('id-ID')} data terdistribusi ke UH
                  </p>
                </div>
              </div>

              {Object.entries(distributeResult.per_kios).map(([kiosId, kiosData]) => (
                <div key={kiosId} className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-hidden">
                  <div className="border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 px-4 py-2.5">
                    <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                      Kios {kiosId}
                      <span className="ml-2 text-xs font-normal text-slate-400">
                        {kiosData.total.toLocaleString('id-ID')} data — {kiosData.uh_count} UH
                      </span>
                    </p>
                  </div>
                  <div className="divide-y divide-slate-100 dark:divide-slate-700">
                    {Object.entries(kiosData.per_uh).map(([uhId, counts]) => (
                      <div key={uhId} className="flex items-center justify-between px-4 py-2.5 text-sm">
                        <span className="font-medium text-slate-600 dark:text-slate-400">UH #{uhId}</span>
                        <div className="flex items-center gap-3 text-xs">
                          <span className="text-fif-600 font-semibold">NMC {counts.nmc}</span>
                          <span className="text-blue-600 font-semibold">REFI {counts.refi}</span>
                          {counts.other > 0 && <span className="text-slate-400">Lain {counts.other}</span>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                <Button variant="secondary" onClick={() => { setShowDistribute(false); setDistributeResult(null); }}>
                  Tutup
                </Button>
              </div>
            </>
