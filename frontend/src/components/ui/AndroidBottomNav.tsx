...
export function AndroidBottomNav() {
  ...
  return (
    <nav
      className="font-poppins fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around px-1 pb-safe pt-2 transition-all duration-300"
      style={{
      ...
      {resolvedTabs.map((tab) => {
        ...
        return (
          <Link
            key={tab.to}
            href={tab.to}
            className={`relative flex flex-1 flex-col items-center gap-1 py-1.5 text-[10px] font-medium transition-all duration-200 ${
              isActive ? 'text-blue-400' : 'text-slate-500 hover:text-slate-300'
            }`}
          >
            {isActive && (
              <motion.span
                layoutId="android-nav-active"
                className="absolute -top-2 h-1 w-6 rounded-full bg-blue-400"
